...
5. 오차(Error)와 손실함수(Loss Function)
6. 경사하강법(Gradient Descent)
7. 머신러닝에서 알고리즘(Algorithm)과 모델(Model)의 차이
---
